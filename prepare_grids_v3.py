...
use_cols = ['CalYear','IncidentGroup','SpecialServiceType','PropertyCategory',
            'Easting_rounded','Northing_rounded','StopCodeDescription','DateOfCall']
...
df['month'] = pd.to_datetime(df['DateOfCall'], dayfirst=True, errors='coerce').dt.month
# Summer: 5-9月, Winter: 11-3月
df['season_summer'] = df['month'].isin([5,6,7,8,9])
df['season_winter'] = df['month'].isin([11,12,1,2,3])
...
def build_grid(subset, sub_configs, label, min_total=2):
    ...
    # 季节统计
    sum_c = subset[subset['season_summer']].groupby(['ge','gn']).size()
    win_c = subset[subset['season_winter']].groupby(['ge','gn']).size()
    summer_counts = {(e,n): int(c) for (e,n), c in sum_c.items()}
    winter_counts = {(e,n): int(c) for (e,n), c in win_c.items()}
    ...
    for _, r in totals.iterrows():
        ...
        props['summer'] = summer_counts.get((e,n), 0)
        props['winter'] = winter_counts.get((e,n), 0)
