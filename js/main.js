...
async function initResponseMap() {
  ...
  mapResp.on('load', () => {
    ...
    // Fire stations - small subtle cross markers, hidden by default
    ...
    document.getElementById('station-toggle')?.addEventListener('click', function() {
      const vis = mapResp.getLayoutProperty('station-dots', 'visibility');
      const show = vis === 'none';
      ...
      if (show) {
        mapResp.setPaintProperty('station-dots', 'circle-opacity', 0.8);
      }
